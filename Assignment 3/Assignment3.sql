-- Q1. Execute following queries on MySQL emp database using Recursive CTEs (not supported in Hive 3.x).
...
-- Q2. Implement movie recommendation in python/java + hive.
...
-- Q3. Create ORC table emp_job_part to partition emp data jobwise. Upload emp data dynamically into these partitions.
...
-- Q4. Create ORC table emp_job_dept_part to partition emp data jobwise and deptwise. Also divide them into two buckets by empno. Upload emp data
...
-- Q5. Load Fire data into Hive in a staging table "fire_staging".
...
LOAD DATA LOCAL INPATH '/home/abhishek/DBDA_DATA/BigData/BigData/data/Fire_Department_Calls_for_Service.csv'
INTO TABLE fire_staging;



-- Q6. Create a transactional ORC table "fire_data" with appropriate data types partitioned by city and buckted by call number into 4 buckets. Load data from
...
-- Q7. Execute following queries on fire dataset.
